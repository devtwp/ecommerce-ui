...
export default async function handler(req, res) {
  ...
  try {
    const { rows } = await pool.query('SELECT * FROM producto_personalizacion');
    res.status(200).json(rows);
  } catch (error) {
    res.status(500).json({ error: 'Error al obtener producto_personalizacion', details: error.message });
  }
}
